...
const useAnimationLoop = (
  ...
  // Use refs to store latest values so animation can access them
  ...
  }, [seqWidth, seqHeight, isHovered, hoverSpeed, targetVelocity]);
  ...
  useEffect(() => {
    ...
    const animate = (timestamp: number) => {
      ...
      const currentTargetVelocity = targetVelocityRef.current;
      const currentHoverSpeed = hoverSpeedRef.current;
      const currentIsHovered = isHoveredRef.current;
      const target = currentIsHovered && currentHoverSpeed !== undefined ? currentHoverSpeed : currentTargetVelocity;

      const easingFactor = 1 - Math.exp(-deltaTime / ANIMATION_CONFIG.SMOOTH_TAU);
      velocityRef.current += (target - velocityRef.current) * easingFactor;

      const currentSeqSize = isVertical ? seqHeightRef.current : seqWidthRef.current;
      if (currentSeqSize > 0) {
    ...
    // Initialize position if we have dimensions
  ...
  ({
    ...
    const updateDimensions = useCallback(() => {
      ...
      if (isVertical) {
        ...
        // Always update width, even if 0, to trigger recalculation
        if (sequenceWidth > 0) {
          setSeqWidth(Math.ceil(sequenceWidth));
          const copiesNeeded = Math.ceil(containerWidth / sequenceWidth) + ANIMATION_CONFIG.COPY_HEADROOM;
          setCopyCount(Math.max(ANIMATION_CONFIG.MIN_COPIES, copiesNeeded));
        } else {
          // If width is 0, still set it to trigger updates, but don't set copyCount
          setSeqWidth(0);
        }
      }
    }, [isVertical]);
